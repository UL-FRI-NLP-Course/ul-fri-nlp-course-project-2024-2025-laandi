...
\section*{Evaluation}
The evaluation of our model is: etc...

\subsection*{Baseline model evaluation}
Evaluation of our model with it's starting template and without searching for new papers, using the data saved on the vectorstore

\subsection*{Evaluation of the model, while searching for new papers}
Evaluation of our model with the new papers, using the data saved on the vectorstore and searching for new papers based on the keywords given in the query.

\subsection*{Comparison with other agents}
In this section, we compare our model's performance with other conversational agents like ChatGPT and Claude.
...
The results of our testing suite show that our model is able to provide relevant and accurate answers to the questions posed, while also being able to search for new papers and integrate them into the answers.
...
\subsection*{Question 4: Tell me everything you know about unsupervised learning in NLP. Can you give me some references or links I can check also?}
\begin{itemize}
  \item \textbf{Without New Papers:}
  Unsupervised learning in NLP involves discovering patterns in large, unlabeled text datasets. Common techniques include: \\
  - \textbf{Topic Modeling:} Identifies latent topics in document collections. \\
  - \textbf{Document Clustering:} Groups similar documents. \\
  - \textbf{Word Embeddings:} Represents words in vector space to capture meaning.

  Algorithms used: LDA, NMF, Word2Vec. \\
  Suggested readings:
  \begin{itemize}
    \item \textit{Unsupervised Learning for Text Mining} – S. Datta, A. Ghosh
    \item \textit{Topic Modeling for Text Mining} – M. Blei, L. Lafferty
    \item \textit{Word Embeddings for Text Mining} – M. Mikolov, K. Sutskever
  \end{itemize}
  
  \item \textbf{With New Papers:}
  Similar to above, with focus on open-source tools like NLTK, spaCy, and Gensim for implementation. Emphasizes topic modeling and clustering along with modern embeddings.

\end{itemize}

\subsection*{Question 5: I want to start learning about RAG in Natural Language Processing. What literature and articles do you suggest I go through? Can you give me dates of their release as well?}
...
\subsection*{Question 6: What can you tell me about semantic web? Give me longer, more informed answer.}
